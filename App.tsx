  ...
  if (appState === 'LANDING') {
    return (
      ...
      >
        ...
        <div className="relative z-10 bg-white rounded-3xl shadow-2xl flex flex-col md:flex-row overflow-hidden max-w-5xl w-full min-h-[600px] border border-white/10">
          
          {/* LEFT SIDE: HERO & VIEW MODE */}
          <div className="w-full md:w-1/2 p-12 flex flex-col items-center justify-center text-center bg-slate-50 relative overflow-hidden">
            {/* Background Pattern */}
            <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-blue-600 via-red-600 to-orange-500"></div>
            
            <div className="mb-10 relative z-10">
               <div className="h-28 w-auto mb-6 flex flex-col items-center justify-center">
                 <img 
                    src="./logo.png" 
                 ...
                 />
                 {/* Fallback Text */}
                 <div className="text-3xl font-bold text-slate-800 hidden peer-[:hidden]:block tracking-tight">
            ...
            <p className="text-slate-500 mb-12 max-w-xs font-medium leading-relaxed">
              Platform terintegrasi untuk pemantauan pelanggaran, blokir, dan pembinaan Awak Mobil Tangki.
            </p>

            <button 
              onClick={() => setAppState('VIEW')}
              className="group relative w-full max-w-xs bg-blue-900 hover:bg-blue-800 text-white font-bold py-4 px-8 rounded-xl shadow-xl transition-all transform hover:-translate-y-1 overflow-hidden"
            >
          ...
          <div className="w-full md:w-1/2 p-12 flex flex-col justify-center bg-white relative">
            <div className="max-w-sm mx-auto w-full">
              <div className="mb-8">
                  <h2 className="text-2xl font-bold text-slate-900">Login Administrator</h2>
                  <p className="text-sm text-slate-500 mt-1">Masuk untuk mengelola data pelanggaran.</p>
              </div>
              ...
              <form onSubmit={handleLogin} className="space-y-5">
                <div>
                  ...
                  <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full pl-10 bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-lg focus:ring-2 focus:ring-blue-800 focus:border-blue-800 block p-3.5 transition-all outline-none"
                        placeholder="EPN-XXX"
                      />
                  </div>
                </div>
                ...
                {loginError && (
                  <div className="p-3 text-sm text-red-600 bg-red-50 border border-red-100 rounded-lg animate-shake flex items-center gap-2">
                ...
                <button 
                  type="submit" 
                  className="w-full text-white bg-red-600 hover:bg-red-700 focus:ring-4 focus:ring-red-200 font-bold rounded-lg text-sm px-5 py-4 text-center transition-all shadow-lg transform hover:-translate-y-0.5 mt-4"
                >
                  MASUK SYSTEM
                </button>
              </form>
    ...
    <div className="min-h-screen font-sans bg-slate-50 flex flex-col">
      ...
      {appState === 'VIEW' && (
          <header className="bg-white shadow-sm border-b border-blue-900/10 sticky top-0 z-40">
            <div className="max-w-[1600px] mx-auto px-6 h-18 flex justify-between items-center py-3">
              <div className="flex items-center cursor-pointer gap-4 group" onClick={handleGoHome}>
                <div className="h-10 w-auto">
                    <img 
                        ...
                        onError={(e) => { e.currentTarget.style.display = 'none'; }} 
                    />
                    <div className="text-2xl font-bold text-slate-800 hidden peer-[:hidden]:block group-hover:scale-105 transition-transform">
                        elnusa <span className="text-red-600">petrofin</span>
                    </div>
                </div>
              ...
              <div className="flex items-center gap-4">
                  <button 
                    ...
                    className="text-sm font-bold text-slate-500 hover:text-blue-800 transition-colors flex items-center gap-2 px-4 py-2 hover:bg-slate-50 rounded-lg"
                  >
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
                    Home
                  </button>
                  <div className="h-8 w-[1px] bg-slate-200"></div>
                  <button onClick={() => setAppState('LANDING')} className="bg-blue-900 text-white text-sm font-bold px-5 py-2 rounded-lg hover:bg-blue-800 shadow-md transition-all">
                      Login Admin
                  </button>
              </div>
            </div>
